...
class FLAME(nn.Module):
    ...
    def __init__(self, flame_model_path,static_landmark_embedding_path,batch_size, use_face_contour=False,shape_params=100,expression_params=50,use_3D_translation=False):
        ...
        default_shape = torch.zeros(
            [self.batch_size, 300 - shape_params],
            dtype=self.dtype,
            requires_grad=False,
        )
        ...
        default_exp = torch.zeros(
            [self.batch_size, 100 - expression_params],
            dtype=self.dtype,
            requires_grad=False,
        )
        self.register_parameter(
