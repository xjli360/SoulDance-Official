...
if __name__ == "__main__":
    ...
    joints_num = 52
    # ds_num = 8
    file_dir_path = '/workspace/SoulNet/dataset/souldance/'
    # change your motion_data joint
    data_dir = os.path.join(file_dir_path, 'joint')
    # change your save folder
    save_dir1 = os.path.join(file_dir_path, 'new_joints')
    # change your save folder
    save_dir2 = os.path.join(file_dir_path, 'motion_623')
    ...
    example_data = np.load('/workspace/SoulNet/dataset/souldance/joint/724830_737558_slice178.npy')
    ...
    for source_file in tqdm(source_list):
        ...
        try:
            ...
            os.makedirs(os.path.split(source_file.replace(
                'joint', 'motion_623'))[0], exist_ok=True)
            np.save(source_file.replace('joint', 'new_joints'),
                    rec_ric_data.squeeze().numpy())
            np.save(source_file.replace('joint', 'motion_623'), data)
    ...
    print('Total clips: %d, Frames: %d, Duration: %fm' %
          (len(source_list), frame_num, frame_num / 20 / 30))
